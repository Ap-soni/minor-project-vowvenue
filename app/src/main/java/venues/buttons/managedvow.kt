...
import com.example.minor1.databinding.FragmentManagedvowBinding
import com.google.firebase.firestore.FirebaseFirestore
import models.profile.cateringsinglemodel
import recyclerviews.venueadapter
import recyclerviews.venueadapter2

class managedvow : Fragment() {

    lateinit var binding: FragmentManagedvowBinding
    ...
    ): View? {  binding= DataBindingUtil.inflate(inflater,R.layout.fragment_managedvow,container,false)
        venuelists = ArrayList()
        venuelistsecond=ArrayList()
        firestore = FirebaseFirestore.getInstance()

        binding.recyclerViewvenue.setHasFixedSize(true)
        binding.recyclerViewvenue.layoutManager= LinearLayoutManager(requireContext(),
