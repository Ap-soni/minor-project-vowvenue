...
import com.example.minor1.databinding.FragmentAdminUploadCateringBinding
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.firestore.FirebaseFirestore
import com.google.firebase.storage.FirebaseStorage
import models.datamodels.cateringonclickmodel

class admin_upload_catering : Fragment() {
    ...
    lateinit var binding:FragmentAdminUploadCateringBinding


    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {

        binding=DataBindingUtil.inflate(inflater,R.layout.fragment_admin_upload_catering,container,false)
       ...
       binding
        ...
        binding.savedetails.setOnClickListener {
            ...
            val cuisines=binding.cuisinesservice.text.toString()
            val workingstyle=binding.workingStyle.text.toString()
            ...
            }else if(TextUtils.isEmpty(cuisines)){
                Toast.makeText(requireContext(),"cuisines is Empty",Toast.LENGTH_SHORT).show()
            }else if(TextUtils.isEmpty(workingstyle)){
     ...
     fun savecatering() {
         ...
         val storageReference= FirebaseStorage.getInstance().getReference().child("caterings images").child("$name").child("image")
         storageReference.putFile(imageuri!!).addOnSuccessListener {

             storageReference.downloadUrl.addOnCompleteListener {uriTask->
                 var imageuri1=uriTask.result.toString()
                 val catering = cateringonclickmodel(name,location, nonvegper, vegper, cuisines, description, experience, mincapacity, workingstyle, rating, phone,
                     imageuri1!!
                 )

                 firestore.collection("caterings").add(catering).addOnSuccessListener {
